  ...
  head: () => ({
    meta: [
      { title: "Events & Activities in Luxor — Luxor AI" },
      {
        name: "description",
        content:
          "Festivals, concerts, opera, balloon ascensions and pilgrimages — the cultural calendar of Luxor.",
      },
      {
        property: "og:title",
        content: "Events & Activities in Luxor — Luxor AI",
      },
      {
        property: "og:description",
        content: "What's on in Luxor, Egypt.",
      },
    ],
  }),
      ...
      <div className="mt-14 grid gap-7 md:grid-cols-2 lg:grid-cols-3">
        {(data ?? []).map((e) => {
          ...
          return (
            <article
              key={e.id}
              className="overflow-hidden rounded-2xl border border-border/60 bg-card shadow-elegant"
            >
              {/* Image */}
              <div className="relative aspect-[4/3] overflow-hidden">
                <img
                  src={imageForEvent(e.category)}
                  alt={e.title}
                  loading="lazy"
