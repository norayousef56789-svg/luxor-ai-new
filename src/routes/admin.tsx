...
export const Route = createFileRoute("/admin")({
  head: () => ({
    meta: [{ title: "Admin Dashboard — Luxor AI" }],
  }),
  component: AdminGate,
});
...
function AdminGate() {
  ...
  // IMPORTANT:
  // Only redirect to /auth when the user is NOT logged in.
  useEffect(() => {
    if (!loading && !user) {
      navigate({
        to: "/auth",
  ...
  // While checking the session, don't render anything yet.
  if (loading) {
    return (
  ...
  // If there is no logged-in user, the effect above will redirect.
  if (!user) return null;

  // Logged-in admin → show the dashboard.
  ...
  return (
    <div className="mx-auto max-w-md px-6 py-24">
      <div className="text-center">
        <div className="inline-grid h-12 w-12 place-items-center rounded-full bg-gold/15 text-gold">
        ...
        <p className="mt-3 text-sm text-muted-foreground">
          ...
          but do not have admin privileges.
        </p>
      </div>
      ...
      {adminCount === 0 ? (
        <div className="mt-8 rounded-2xl border border-gold/40 bg-gold/5 p-6 text-center">
          ...
          <p className="mt-2 text-xs text-muted-foreground">
            No admin exists yet. The first signed-in user can claim admin
            access.
          </p>
        ...
        {tab === "approvals" && (
          <div className="space-y-4">
            ...
            {pending.map((b) => (
              <div
                key={b.id}
                className="rounded-xl border border-border/60 bg-card/50 p-5 flex items-start justify-between gap-4 flex-wrap"
              >
                <div>
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="inline-flex items-center gap-1 rounded-full bg-gradient-gold px-4 py-2 text-sm text-primary-foreground"
                  >
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-1 rounded-full border border-destructive/50 text-destructive px-4 py-2 text-sm"
                  >
                    <X className="h-4 w-4" />
                    Reject
                  </button>
        ...
        {tab === "offers" && (
          <div className="space-y-3">
            {(offers ?? []).map((o) => (
              <div
                key={o.id}
                className="flex items-start justify-between gap-3 rounded-xl border border-border/60 bg-card/50 p-4"
              >
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    {o.description && (
                      <p className="text-xs text-foreground/70 mt-1 max-w-xl">
                        {o.description}
                      </p>
                    )}
                  </div>
...
function StatusPill({ status }: { status: string }) {
  ...
  return (
    <span
      className={`text-xs rounded-full px-2 py-0.5 border ${cls}`}
    >
      {status}
    </span>
  );
    ...
    <div className="space-y-4">
        ...
        >
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              placeholder="Music, Festival, Adventure…"
            />
          </div>
          ...
          <button className="md:col-span-2 rounded-full bg-gradient-gold px-4 py-2 text-sm text-primary-foreground shadow-gold">
            Publish event
          </button>
        </form>
      )}
      ...
      <div className="grid gap-3 md:grid-cols-2">
        {(data ?? []).map((e) => (
          <div
            key={e.id}
            className="rounded-xl border border-border/60 bg-card/50 p-4 flex items-start justify-between gap-3"
          >
            <div>
